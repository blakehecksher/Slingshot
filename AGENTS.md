...
1. Check `docs/spec/` for the most recent root spec file. That's current intent unless `docs/state.md` already exists and says otherwise.
...
3. Read the active specs named in `docs/state.md`. For the current Slingshot direction, that means:
   - `docs/spec/slingshot-time-trials-product-outline.md`
   - `docs/spec/slingshot-lore-and-visual-direction.md`
4. Check `docs/plans/` for the active plan linked in state.md. Follow it unless you have a specific reason not to. If you diverge, log why.
5. Begin work.
...
```text
docs/
|-- spec/          Active product/lore specs live at root. Historical or superseded specs live in archive/.
...
Do not treat `docs/spec/archive/` as current truth unless the user explicitly asks to revive, compare, or distill archived material.
